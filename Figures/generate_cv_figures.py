...
for i, comp in enumerate(competitors):
    if nrows >= 2:
        ...
        ax[i % ncols].plot([limit_min, limit_max], [
                           limit_min, limit_max], color='r')
        ax[i % ncols].plot([limit_min, limit_max-margin], [limit_min +
                           margin, limit_max], color='orange', linestyle='--', alpha=0.75)
        ax[i % ncols].plot([limit_min + margin, limit_max], [limit_min,
                           limit_max-margin], color='orange', linestyle='--', alpha=0.75)
        ax[i % ncols].annotate(
            '{}'.format(margin),
            (limit_min + margin/2 + 0.0125, limit_min+margin/2 - 0.01),
            fontsize=12
        ...
        ax.plot([limit_min, limit_max-margin], [limit_min + margin,
                limit_max], color='orange', linestyle='--', alpha=0.75)
        ax.plot([limit_min + margin, limit_max], [limit_min,
                limit_max-margin], color='orange', linestyle='--', alpha=0.75)
        ax.arrow(limit_min + margin, limit_min, -margin/2, +margin/2,
                 length_includes_head=True, color='orange', alpha=0.75, head_width=0.0075)
        ax.annotate(
            '{}'.format(margin),
            (limit_min + margin/2 + 0.0125, limit_min+margin/2 - 0.01),
            fontsize=12
