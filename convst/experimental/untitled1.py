...
def generate_shapelet(X, y, l, _seed, alpha_stop):
    """
    ...
    base_size = int64(max_d*sqrt(n_samples*n_timestamps))
    ...
    for d in range(1, max_d+1):
        #Length of the distance vector given l,d
        d_size = n_timestamps - (l-1) * d
        #For all samples, we have the weigth array which we use to sample shapelets
        w_size = n_samples * d_size
        ...
        for norm in range(0,2):
            ...
            while True:
                ...
                #i_in = numba_choice_p(arange(w_size), W)
                i_in = np.argmax(W)
                ix = i_in//d_size
                ij = i_in%d_size        
                val = _get_subsequence(X[ix, 0], ij, l, d, norm)
                D = compute_distance_vectors(X, val, l, d, norm)
                ix_same_y = choice(np.where(y==y[ix])[0])
                th = np.random.uniform(
                    np.percentile(D[ix_same_y], 5), np.percentile(D[ix_same_y], 10)
                )
                diff = 1
                diff1 = 1
                ...
                #print("1-{}/{} = {}".format(diff,diff1,1-diff/diff1))
                if 1-(diff/diff1) < alpha_stop:
                    break
                i_shp += 1
...
class IR_DST2(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        out, values, lengths, dilations, threshold, normalize = generate_shapelet(
            X, y, shapelet_sizes[0], seed, self.alpha_stop
        )
        self.values_ = values
