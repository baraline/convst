...
def DN_HistogramMode_5(X):
    ...
    return np.float64(_histogram_mode(X, 5, X.min(), X.max()))

@njit(cache=True, fastmath=True)
def DN_HistogramMode_10(X):
    # Mode of z-scored distribution (10-bin histogram).
    return np.float64(_histogram_mode(X, 10, X.min(), X.max()))
...
def SP_Summaries_welch_rect_area_5_1(X):
    ...
    return np.float64(_summaries_welch_rect(X, False, fft_1d(X)))

@njit()
def SP_Summaries_welch_rect_centroid(X):
    # Centroid of the Fourier power spectrum.
    return np.float64(_summaries_welch_rect(X, True, fft_1d(X)))
...
@njit(fastmath=True, cache=True)
def CO_HistogramAMI_even_2_5(X):
...
def SB_BinaryStats_mean_longstretch1(X):
    ...
    return np.float64(_long_stretch(diff_binary, 0))
...
def SC_FluctAnal_2_dfa_50_1_2_logi_prop_r1(X):
    ...
    return np.float64(_fluct_prop(cs, len(X), True))
...
def SC_FluctAnal_2_rsrangefit_50_1_logi_prop_r1(X):
    ...
    return np.float64(_fluct_prop(cs, len(X), False))
