    ...
    while continue_search:
        ...
        for i in prange(n_samples):
            for j in prange(dist_vect_shape):
                ...
                if _min_ce > 0:
                    candidate_distances[i,j] *= (max(CE_shp,CEs[i,j]) / _min_ce)
                else:
                ...
                for i in prange(n_samples):
                    ...
                    for j in prange(len(ix_shapelets)):
                        ...
                        X_new[i, (n_features * i_shp):(n_features * i_shp + n_features)] = apply_one_shapelet_one_sample(
                            X_sample[normalize[i_shp]], values[i_shp, :lengths[i_shp]], threshold[i_shp]
                        )
    return X_new
...
def apply_one_shapelet_one_sample(x, values, threshold):
    ...
    _min = 1e+100
    _argmin = 0
    _ce_shp = _CE(values,values.shape[0])
    
    # For each step of the moving window in the shapelet distance
    for i in range(n_candidates):
        _ce_x = _CE(x[i],x.shape[1])
        _min_ce = min(_ce_x, _ce_shp)
        if _min_ce > 0:
            CF = max(_ce_x, _ce_shp) / _min_ce
        _dist = 0
