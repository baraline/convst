...
class ConvolutionalShapeletTransformer_onlyleaves(BaseEstimator, TransformerMixin):
    ...
    Notes
    -----
    .. [1] Guillaume A. et al, "Convolutional Shapelet Transform: A new approach of time series shapelets" (2021)
    ...
    def fit(self, X, y):
        ...
        self._log("Extracting shapelets from {} leaves ...".format(
            id_X_leaves.shape[0]))
        shp, d = _extract_candidates(X, id_X_leaves, kernel_id, L, dils, self.P)
        del id_X_leaves
        del kernel_id
        gc.collect()
        ...
        self.shapelets = np.zeros((n_shapelets, 9), dtype=np.float64)
    ...
    def _discretize(self, candidates):
        ...
        print("NAN : {}".format(np.isnan(np.sum(candidates))))
        if not np.all(candidates.reshape(-1, 1) == candidates.reshape(-1, 1)[0]):
            kbd = KBinsDiscretizer(n_bins=self.n_bins, strategy='uniform', dtype=np.float64).fit(
    ...
    def _generate_nodes(self, ft, y):
        ...
        id_X_leaves = np.zeros((n_leaves.sum(), ft.shape[0]), dtype=np.int8) - 1
        ...
        for tree_id in range(self.n_trees):
            ...
            for i, id_leaf in enumerate(id_leaves):
                parent_id = np.where((tree.tree_.children_right == id_leaf) | (tree.tree_.children_left == id_leaf))[0][0]
                id_X_leaves[i_split_trees[tree_id]+i,
                            node_indicator[:, parent_id].nonzero()[0]] += 2
                id_X_leaves[i_split_trees[tree_id]+i,
                            node_indicator[:, id_leaf].nonzero()[0]] -= 1
                kernel_id[i_split_trees[tree_id]+i] += tree.tree_.feature[parent_id]
    ...
    def transform(self, X, return_inverse=False):
        ...
        X = check_array_3D(X)
        distances = np.zeros(
            (X.shape[0], self.shapelets.shape[0]), dtype=np.float64)
        prev = 0
...
def _extract_candidates(X, id_X_leaves, kernel_id, L, dils, P):
    ...
    candidates = np.zeros((n_candidates, 9), dtype=np.float64)
...
def _extract_location_mask(id_X_leaves, kernel_id, dils, L, P, n_timestamps):
    ...
    for i_k in prange(unique_kernels.shape[0]):
        ...
        for i_leaf in prange(id_leaves.shape[0]):
            ...
            LC = np.zeros((2, Lp.shape[1]), dtype=np.float64)
