...
def _init_random_shapelet_params(n_shapelets, shapelet_sizes, n_timestamps, p_norm):
    ...
    # Dilations
    dilations = np.ones(n_shapelets, dtype=np.int64)
...
@njit(cache=True, parallel=True)
def generate_shapelet(X, y, n_shapelets, shapelet_sizes, seed, p_norm):
    """
    ...
    -------
    set of arrays
        Return the values, lengths, dilations, and normalization
    ...
    values, lengths, dilations, normalize = _init_random_shapelet_params(
    ...
    return values, lengths, dilations, normalize.astype(np.int64)


@njit(cache=True, parallel=True, fastmath=True, error_model='numpy')
def apply_all_shapelets(X, values, lengths, dilations, normalize):
    """
...
def apply_one_shapelet_one_sample(x, values):
    ...
    return _min, np.float64(_argmin)


class R_ST_NL(BaseEstimator, TransformerMixin):
    """
    ...
    def fit(self, X, y):
        ...
        values, lengths, dilations, normalize = generate_shapelet(
    ...
    def transform(self, X):
        ...
        check_is_fitted(self, ['values_', 'length_',
                        'dilation_', 'normalize_'])
        X_new = apply_all_shapelets(
            X, self.values_, self.length_, self.dilation_, self.normalize_
        )
        return X_new
