...
def generate_shapelet(X, y, n_shapelets, shapelet_sizes,
                      seed, p_norm, p_min, p_max, max_channels):
    """
    ...
    values, lengths, dilations, threshold, normalize, channels = _init_random_shapelet_params(
        n_shapelets, shapelet_sizes, n_timestamps, p_norm, n_features, max_channels
    )
    ...
    for i in prange(n_shapelets):
        ...
        x_dist = np.zeros(n_timestamps - (lengths[i]-1)*dilations[i])
...
def apply_all_shapelets(X, values, lengths, dilations, threshold,
    ...
    """
    n_samples, n_ft, n_timestamp = X.shape
...
class MR_DST(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, n_shapelets=10000, shapelet_sizes=[11], max_channels=None,
                 random_state=None, p_norm=0.8, percentiles=[5, 10], n_jobs=1):
    ...
    def fit(self, X, y=None):
        ...
        values, lengths, dilations, threshold, normalize, channels = generate_shapelet(
            ...
            self.percentiles[0], self.percentiles[1], self.max_channels
        )
        self.values_ = values
    ...
    def transform(self, X):
        ...
        X_new = apply_all_shapelets(
            ...
            self.normalize_, self.channels_
        )
        return X_new
