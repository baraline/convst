...
def generate_shapelet(
        ...
        p_min, p_max, alpha, max_channels
    ):
    """
    ...
    for i in prange(n_shapelets):
        ...
        _i_timestamp = np.random.choice(n_timestamps)
        
        _i_test = np.random.choice(np.where(y==y[_i_sample])[0])
        _x_dist = np.zeros(n_timestamps)
        
        a3 = 0  # for weights (per channel)
        for _i_c in range(_n_channels):
            b3 = a3 + _length
            _v = _get_subsequence(
                X[_i_sample, _channel_ids[_i_c], :n_timestamps], _i_timestamp,
                _length, _dilation, _norm
            )
            if _norm:
                _x_dist += compute_shapelet_dist_vector_norm(
                    X[_i_test, _channel_ids[_i_c], :n_timestamps], _v, _length, _dilation    
                )
...
def apply_all_shapelets(X, shapelets):
    """
    ...
    for i_sample in prange(n_samples):
        ...
        for i_shp_param in prange(n_shp_params.shape[0]-1):
            ...
            strides = _generate_strides_2D_phase(
                X[i_sample, :, :n_timestamps], _length, _dilation
            )
