...
from CST.utils.shapelets_utils import generate_strides_2D, shapelet_dist_numpy, generate_strides_1D, euclidean_pure_numpy
...
class ConvolutionalShapeletTransformer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        n_shp = 0
        dils = dils[kernel_id]
        for dil in np.unique(dils):
            candidates = []
            for i_split in np.where(dils==dil)[0]:
                mask = X_split[i_split]
                k_id = kernel_id[i_split]
                y_splt = y_split[i_split][mask]
                classes = np.unique(y_splt)
                n_classes = classes.shape[0]
                Lp = _generate_strides_2d(L[mask, k_id, :], 9, dil).sum(axis=-1)
                if self.use_class_weights:
                    c_w = compute_class_weight('balanced', classes=classes, y=y_splt)
                else:
                    c_w = np.ones(2)
                ...
                for i_class in classes:
                    if LC.sum() > 0:
                        ...
                        for i_r in range(len(regions)):
                            ...
                            x_index = np.argmax(
                                Lp[y_splt==i_class, id_max_region])
                            candidates.append(X[np.where(y_splt == i_class)[0][x_index], 0, np.array(
                                [id_max_region+j*dil for j in range(9)])])
            ...
            if candidates.shape[0] > 0:
                ...
                shapelets.update({dil: candidates})
                n_shp += candidates.shape[0]
    ...
    def fit2(self,X,y):
        L, dils, X_split, y_split, kernel_id = self._generate_inputs(X,y)
        arr = np.concatenate((X_split,y_split,kernel_id.reshape(-1,1)),axis=1)
        u, idx = np.unique(arr,return_index=True,axis=0)
        X_split = X_split[idx]
        y_split = y_split[idx]
        kernel_id = kernel_id[idx]
        shapelets = {}
        n_shp = 0
        
        shp, d = _fit(X, X_split, y_split, kernel_id, L, dils, self.P)
        
        self.n_shapelets=shp.shape[0]
        self.shapelets = {dil:shp[d==dil] for dil in np.unique(d)}
    ...
    def _generate_inputs(self, X, y):
        ...
        n_nodes = np.asarray([rf.estimators_[i_dt].tree_.node_count for i_dt in range(self.n_trees)])
        n_leaves = np.asarray([rf.estimators_[i_dt].tree_.n_leaves for i_dt in range(self.n_trees)])
...
def _fit(X, X_split, y_split, kernel_id, L, dils, P):
    x_mask = np.zeros((X_split.shape[0],2,X.shape[0]*X.shape[2]),dtype=np.int32)
    ...
    for i_split in prange(X_split.shape[0]):
        ...
        classes = np.unique(y_splt)
        for i_class in classes:
            indexes = np.where(x_mask[i_split,i_class]>0)[0]
