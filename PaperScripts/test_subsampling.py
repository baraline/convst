...
csv_name = 'CV_{}_results_subs.csv'.format(
    n_cv)
...
dict_models = {
    "RDST":R_DST,
    "R_DST_Sampling_0.5":R_DST_Sampling,
    "R_DST_SubSampling_0.5":R_DST_Subsampling,
    "R_DST_Sampling_1.0":R_DST_Sampling,
    "R_DST_SubSampling_1.0":R_DST_Subsampling
}
    ...
    for model_name, model_class in dict_models.items():
        if pd.isna(df.loc[i_df, 'acc_mean']) or df.loc[i_df, 'acc_mean'] == None or df.loc[i_df, 'acc_mean'] == 0.0:
            if model_name == 'RDST':
                ...
                if model_name[-3:] == '1.0':
                    alpha = 1.0
                print(model_name +' '+str(alpha))
                pipeline_RDST_rdg = make_pipeline(
                    model_class(n_jobs=90, alpha=alpha), 
                    c_StandardScaler(with_mean=True),
                    RidgeClassifierCV(alphas=np.logspace(-3,3,20))
                )
