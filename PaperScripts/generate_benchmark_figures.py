...
df = pd.read_csv(n_samples, sep=',').set_index('Unnamed: 0').rename(columns={'cst':'CST',
                                                                                 'rkt':'MiniRKT',
                                                                                 'sfc':'SFC'})
df2 = pd.read_csv(n_timepoints, sep=',').set_index('Unnamed: 0').rename(columns={'cst':'CST',
                                                                                 'rkt':'MiniRKT',
                                                                                 'sfc':'SFC'})
...
ax[1].add_patch(a3)
ax[1].text(df2.index[-1]-65000, (df2.loc[df2.index[-1],'MiniRKT']+df2.loc[df2.index[-1],'CST'])//10,'x{}'.format(int(df2.loc[df2.index[-1],'CST']//df2.loc[df2.index[-1],'MiniRKT'])))