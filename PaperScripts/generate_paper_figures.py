...
k = Rocket_kernel(length=9, bias=biases[i_kernel], dilation=dils[i_kernel], padding=0, weights=weights[i_kernel], id_ft=0)
...
for c in np.unique(y_train):
    ...
    x = (x-x.mean())/x.std()
    ax[c].plot(x, color='orange' if c==1 else 'blue',alpha=0.5,label='class {}'.format(c))
...
for i_r in CST._get_regions(id_w):
    ax[2].fill_between(i_r, D[i_r],np.percentile(D,80),color='red',alpha=0.75)
ax[2].set_title('LC difference')

ax[3].plot(X_train[np.where(y_train==0)[0][0],0],c='blue',label='class 0')
ax[3].plot(X_train[np.where(y_train==1)[0][0],0],c='orange',label='class 1')
...
for i_class in np.unique(y_test):
    for i, i_grp in enumerate(CST.shapelets_params.keys()):
        ...
        for j in range(i_shp.shape[0]):
            d = np.abs(x_strides - CST.shapelets_values[i][i_shp[j]]).sum(axis=1)
            dists = d.min()
            loc = d.argmin()
...
cmap = matplotlib.cm.ScalarMappable(norm=norm,cmap=matplotlib.cm.get_cmap('jet'))
fig, ax = plt.subplots(ncols=2,sharex=True, sharey=True, figsize=(15,5))
